...
async function seed() {
  ...
  await tx(async (client) => {
    ...
    for (const [nome, codigo, cozinha, categoria] of pdvs) {
      await client.query(
        `INSERT INTO pdvs (nome, senha, codigo_orion, is_cozinha, categoria)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (nome) DO UPDATE SET codigo_orion = EXCLUDED.codigo_orion, is_cozinha = EXCLUDED.is_cozinha, categoria = EXCLUDED.categoria`,
        [nome, hashPassword("123456"), codigo, cozinha, categoria]
      );
    }
    ...
    for (const [sku, nome, qtd, categoria] of produtos) {
      await client.query(
        `INSERT INTO produtos (sku, nome, qtd_total, estoque_central, ativo, categoria, origem)
         VALUES ($1, $2, $3, $3, TRUE, $4, 'omie')
         ON CONFLICT (sku) DO UPDATE SET nome = EXCLUDED.nome, qtd_total = EXCLUDED.qtd_total, ativo = TRUE, categoria = EXCLUDED.categoria, origem = 'omie'`,
