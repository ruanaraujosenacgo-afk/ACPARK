...
test("Supabase Storage adapter masks sensitive data in provider errors", async () => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async () => new Response(
    'authorization=Bearer eyJsecret apikey="service-role-key" postgres://user:pass@host/db',
  ...
  try {
    ...
    await assert.rejects(
      () => adapter.saveFile("avarias/teste/foto.png", PNG_1X1),
      (error) => {
        assert.match(error.message, /removido/);
        assert.doesNotMatch(error.message, /eyJsecret|service-role-key|user:pass/);
        return true;
      }
    );
  } finally {
