...
test("history report can export current point or all points to spreadsheet", () => {
  ...
  assert.match(historyBlock, /downloadCsv\(filename, \[headers, \.\.\.dataRows\]\)/);
  assert.match(historyBlock, /historico_todos_os_pontos\.csv/);
  assert.match(historyBlock, /historico_\$\{slugFileName\(selectedPointLabel\)\}\.csv/);
});
